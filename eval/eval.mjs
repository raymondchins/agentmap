...
//   --refresh         re-fetch fixtures even if already present
//   --repin           clone HEAD instead of the pinned sha and print the new
//                     shas, for updating FIXTURES deliberately
...
// Fixtures are PINNED. Without a sha this cloned `--depth 1` at whatever HEAD
// happened to be that day and merely *recorded* the result, so two runs measured
// two different codebases — and a number moving between runs said nothing about
// whether agentmap got better or the upstream repo did. Published accuracy
// figures have to be re-derivable, so the sha is an input, not an output.
//
// Re-pin deliberately with `--repin`: it clones HEAD, prints the shas, and
// changing them here is then a reviewable diff that dates itself.
//
// FULL 40-char shas are required: GitHub refuses `git fetch <abbreviated-sha>`
// (it serves fetch-by-sha only for the complete object id).
const FIXTURES = [
  { name: "zod", url: "https://github.com/colinhacks/zod", sourceRoot: "packages/zod/src/v4", sha: "912f0f51b0ced654d0069741e7160834dca742ee" },
  { name: "zustand", url: "https://github.com/pmndrs/zustand", sourceRoot: "src", sha: "beca84e600e4e250f6b244d22878e72948f331c7" },
  { name: "hono", url: "https://github.com/honojs/hono", sourceRoot: "src", sha: "26d8e42bfff7eb78fc323a01e1d02d63b9dbcc72" },
];
...
const REPIN = has("--repin");
// --repin implies a fresh clone: the whole point is to move off the pinned sha.
const REFRESH = has("--refresh") || REPIN;
...
function ensureClone(fx) {
  ...
  if (existsSync(dir) && !REFRESH) {
    ...
    if (fx.sha && !REPIN) {
      // Fetch exactly the pinned commit. A shallow clone cannot check out an
      // arbitrary older sha, so fetch that one object directly — same single-
      // commit cost as `clone --depth 1`, but deterministic.
      process.stderr.write(`  fetching ${fx.name} @ ${fx.sha} (${fx.url}) ...\n`);
      mkdirSync(dir, { recursive: true });
      runCap("git", ["-C", dir, "init", "-q"]);
      runCap("git", ["-C", dir, "remote", "add", "origin", fx.url]);
      const { status } = runCap("git", ["-C", dir, "fetch", "--depth", "1", "origin", fx.sha]);
      if (status !== 0) throw new Error(`fetch failed for ${fx.name}@${fx.sha} — network, or the commit was garbage-collected upstream (${fx.url})`);
      runCap("git", ["-C", dir, "checkout", "-q", "FETCH_HEAD"]);
    } else {
      process.stderr.write(`  cloning ${fx.name} @ HEAD (${fx.url}) ...\n`);
  ...
  if (REPIN) process.stderr.write(`  REPIN ${fx.name}: sha: "${sha}"\n`);
  else if (fx.sha && !sha.startsWith(fx.sha)) {
    throw new Error(`${fx.name} is at ${sha} but the pin says ${fx.sha} — delete tmp/eval/${fx.name} or pass --refresh`);
  }
  return { dir, sha };
}
...
<!-- /eval:stamp -->
>
> Fixtures are **pinned to exact commits** (see \`FIXTURES\` in \`eval/eval.mjs\`), so
> these numbers are re-derivable rather than a snapshot of whatever upstream looked
> like that day. A figure moving between runs therefore reflects a change in
> agentmap, not in the fixture. Re-pin deliberately with \`--repin\`.
...
`;
  writeEvalDoc(join(ROOT, "EVAL.md"), md);
}

// Write EVAL.md WITHOUT destroying hand-written analysis.
//
// This used to be a wholesale `writeFileSync(EVAL.md, md)`, which meant every
// `npm run eval` silently deleted anything a human had added — including the
// paragraph reconciling this file's "grep is cheaper on tokens" row with
// RESULTS.md's 99.2% blast-radius row. That paragraph is the single most important
// honesty artifact here: it explains why two headline numbers that look contradictory
// are both true. Losing it on a routine regeneration is how an eval stops being
// trustworthy.
//
// So: only the marker-delimited regions are machine-owned. Everything outside them
// belongs to whoever wrote it and survives. A file missing the markers (or missing
// entirely) is bootstrapped from the template — which is also the escape hatch if
// the structure ever needs a reset.
const REGIONS = ["stamp", "overall", "fixtures"];
function writeEvalDoc(path, generated) {
  const region = (src, name) => {
    const m = src.match(new RegExp(`<!-- eval:${name} -->[\\s\\S]*?<!-- /eval:${name} -->`));
    return m ? m[0] : null;
  };

  if (!existsSync(path)) { writeFileSync(path, generated); return; }
  const current = readFileSync(path, "utf8");

  const missing = REGIONS.filter((r) => !region(current, r) || !region(generated, r));
  if (missing.length) {
    // Nothing to splice into. Say so loudly rather than silently overwriting —
    // a quiet full rewrite here is the exact behaviour this function replaced.
    console.error(`# eval: EVAL.md has no ${missing.map((r) => `eval:${r}`).join(" / ")} marker — rewriting the whole file from the template.`);
    console.error("# eval: any hand-written analysis in it is being replaced. Re-add the markers to keep it next time.");
    writeFileSync(path, generated);
    return;
  }

  let out = current;
  for (const r of REGIONS) out = out.replace(region(out, r), region(generated, r));
